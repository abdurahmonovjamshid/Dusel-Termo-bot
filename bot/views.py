...
from .services.addcar import (add_product, add_material, add_measure,
                              add_waste, add_defect, add_quantity, create_excel_report)
...
def cm_start(message):
    try:
        ...
        if str(user.telegram_id) in ADMINS:
            ...
            bot.send_message(
                message.chat.id, "Ma'lumot kiritish boshlandi...", reply_markup=get_back)
            msg = bot.send_message(
                message.chat.id, "Choose a day:", reply_markup=markup)
...
def handle_navigation(call):
    try:
        ...
        if call.data.startswith('infnav_'):
            ...
            reports = Report.objects.filter(date=selected_date).order_by(
                'default_value', 'machine_num')
            ...
            if reports.exists():
                ...
                bot.answer_callback_query(
                    call.id, f"No reports found for {date_str}")

            bot.answer_callback_query(call.id, f"Selected date: {date_str}")
    except Exception as e:
        bot.answer_callback_query(
            call.id, f"{date_str} sana blan ma'lumot kiritilmoqda kuting")
...
def machine_num_callback_handler(call):
    try:
        ...
        bot.edit_message_text(
            ...
            # Assuming next step is product selection
...
def material_selection_handler(call):
    try:
        ...
        report = Report.objects.get(
            is_confirmed=False, user=user)  # Get the report object
        ...
        # Handle material selection
            ...
            bot.edit_message_text(
                chat_id=call.message.chat.id,
                message_id=call.message.message_id,
                text=f"Report\n{report.date} - {report.default_value}\n\n Machine Number: {report.machine_num}Product: {
                    report.product.name}Material: {report.material.name}\n\nTermoplast vazni:",
                parse_mode='html'
            )
            TgUser.objects.filter(telegram_id=call.from_user.id).update(
                step=USER_STEP['ADD_MEASURE'])
        ...
        if call.data.startswith("confirm_"):
            ...
            if confirmed_report:
                ...
                report = Report.objects.create(
                    date=report.date, default_value=report.default_value, user=user)
            msg = bot.send_message(
                chat_id=call.message.chat.id,
                text=f"Report\n{
                    report.date} - {report.default_value}\n\n Select a machine number:",
...
# This catches the "Back" button press
...
def get_step_back(message):
    try:
        ...
        if user.step > USER_STEP['DEFAULT']:
            ...
            bot.send_message(
                chat_id=message.chat.id, text="Malumot kiritish bekor qilindi", reply_markup=main_button)
        ...
        bot.send_message(chat_id=message.chat.id,
                         text="Malumot kiritish bekor qilindi", reply_markup=main_button)
        bot.delete_message(chat_id=message.chat.id, message_id=user.edit_msg)
