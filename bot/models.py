...
class Material(models.Model):
    products = models.ManyToManyField(
        Product, related_name='materials', blank=True)  # Many-to-many relationship
    itemcode = models.CharField(max_length=20, null=True, blank=True)
    name = models.CharField(max_length=100, null=True, blank=True)
...
class Report(models.Model):
    ...
    machine_num = models.IntegerField(
        verbose_name='aparat', null=True, blank=True)
    ...
    defect_measure = models.FloatField(
        blank=True, null=True, verbose_name='atxod')
    waste_measure = models.FloatField(
        blank=True, null=True, verbose_name='brak')
    material = models.ForeignKey(
        Material, on_delete=models.CASCADE, blank=True, null=True)
    quantity = models.FloatField(blank=True, null=True)
    is_confirmed = models.BooleanField(default=False)
    user = models.ForeignKey(
        to=TgUser, on_delete=models.CASCADE, related_name='reports')


class Machine(models.Model):
    number = models.IntegerField(verbose_name="apparat raqami")
    product = models.ForeignKey(
        Product, related_name='machine', on_delete=models.SET_NULL, null=True, blank=True)
